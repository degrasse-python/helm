...
String scriptName = "11-create-ops-controller.groovy"
...
private void createMM(String masterName, def masterDefinition) {
    ...
    Folder opsFolder = jenkins.getItem("operations")
    if (!opsFolder) {
        jenkins.createProject(Folder, "operations")
        opsFolder = jenkins.getItem("operations")
    }
//  jenkins.model.JenkinsLocationConfiguration.get().setUrl("https://sda.ld-test.cb-demos.io/cjoc/")
    ...
    ConfigMap configMap = client.configMaps().inNamespace("cloudbees-sda").withName("cbci-op-casc-bundle").get()
